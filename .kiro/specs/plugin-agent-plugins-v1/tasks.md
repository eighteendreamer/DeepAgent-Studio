...
- [ ] 2. 建立 `plugin/` 模块骨架与规范层常量（M0-3 前置）
...
- [ ] 4. 实现 `PluginName` newtype 与 §5.5 约束（M0-2）
...
- [ ] 7. 实现 `PluginDiagnostic` 并贯通到前端（M0-4）
...
- [ ] 10. 实现 Agent Plugins v1 closed schema 解析（M1-2）
...
- [ ] 13. 实现子进程环境组装与 `PLUGIN_DATA` 契约（M1-5）
...
- [ ] 14. hooks 占位符改走 v1 语义并记录未映射事件（M3 前置，本 spec 内完成）
...
- [ ] 15. 实现方言发现顺序、symlink 拒绝与 overlay 合并（M2-1）
...
- [ ] 18. 接线：`loader` 产出 `ResolvedPlugin`，`runtime` 消费它（M2 收口）
