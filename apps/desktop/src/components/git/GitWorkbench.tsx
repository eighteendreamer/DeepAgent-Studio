    ...
    <div className="flex h-full min-h-0 flex-col bg-white">
      <div className="flex h-12 flex-shrink-0 items-center justify-between border-b border-border-theme px-4">
        ...
        <div className="flex items-center gap-1">
          <TabButton active={tab === "changes"} onClick={() => setTab("changes")} icon={["fas", "list-check"]} label={t("git.tabs.changes")} />
          <TabButton active={tab === "branches"} onClick={() => setTab("branches")} icon={["fas", "code-branch"]} label={t("git.tabs.branches")} />
          <TabButton active={tab === "push"} onClick={() => setTab("push")} icon={["fas", "upload"]} label={t("git.tabs.push")} />
          <TabButton active={tab === "log"} onClick={() => setTab("log")} icon={["fas", "code-commit"]} label={t("git.tabs.log")} />
          {onClose && (
            <button
              type="button"
      ...
      <div className="min-h-0 flex-1">
        {tab === "changes" ? (
          ...
          <GitProjectsPanel activeProjectPath={projectPath} onRefresh={onRefresh} />
        ) : tab === "push" ? (
